...
class ProcesosController extends Controller
{
    ...
    public function updateProductos()
    {
        ...
        if (!file_exists($csvFile) || !is_readable($csvFile)) {
            ...
            $logMessage = $start->toFormattedDateString() . " :: No se encontro ningun archivo para la actualización de Productos";
            ...
            try {
                // dd(storage_path('csv/' . self::DESTINATION_PATH_FILE . '/'), $filename);
                // $this->csvHelper->renameFile(storage_path('csv/' . self::DESTINATION_PATH_FILE . '/'), $filename);
                $path=storage_path('csv/' . self::DESTINATION_PATH_FILE . '/');
                ...
                $logMessage = $start->toFormattedDateString() . " :: Se produjo un error al procesar el archivo de Actualizacion de Productos. Por favor revise el Log de Errores";
            ...
            if ($rows) {
                foreach ($rows as $row) {
                    ...
                    if (count($row) != self::TOTAL_COLS) {
                        ...
                        $logMessage = $start->toFormattedDateString() . " :: Se produjo un error al procesar el archivo de Actualizacion. Por favor revise el Log de Errores";
                        $this->saveMessage($logMessage);
                ...
                $logMessage = $start->toFormattedDateString() . " :: Se proceso el archivo de Actualizaciones correctamente";
                $this->saveMessage($logMessage);
    ...
    private function saveMessage($message)
    {
        ...
        \DB::update('update message set message = ? where id = 1', [$message]);
    ...
    private function createRollBackTableProductos()
    {
    ...
    private function createRollBackTableEquivalencias()
    {
   
        //Equivalencias
        \Schema::dropIfExists('equivalencias_rollback');
        \DB::statement(\DB::raw("CREATE TABLE equivalencias_rollback LIKE equivalencias"));
        \DB::statement(\DB::raw("INSERT equivalencias_rollback SELECT * FROM equivalencias"));

    }

    public function updateEquivalencias()
    {

        $start = \Carbon\Carbon::now();

        Log::info("###################################################################");
        Log::info("Comienza Proceso de actualización de Equivalencias.");
        ...
        if (!file_exists($csvFile) || !is_readable($csvFile)) {
            ...
            Log::info("Finaliza Proceso de actualización de Equivalencias");
            Log::info("     - No se encontro el archivo necesario (equivalencias.csv) para la actualización de Equivalencias en el directorio /storage/csv/repo");
            Log::info("###################################################################" . "\n");

            $logMessage = $start->toFormattedDateString() . " :: No se encontro ningun archivo para la actualización de Equivalencias";
            ...
            try {
                ...
                $logMessage = $start->toFormattedDateString() . " :: Se produjo un error al procesar el archivo de Actualizacion de Equivalencias. Por favor revise el Log de Errores";
                $this->saveMessage($logMessage);
            }

            Log::info("     - Se generan la tabla de RollBack de Equivalencias");
            $this->createRollBackTableEquivalencias();

            $qRegistros = count($rows);
            Log::info("     - El Archivo a procesar posee " . $qRegistros . " registros.");

            $i = 1;
            $table_keys = ['codigo', 'cod_equivalencia'];
            ...
            if ($rows) {
                foreach ($rows as $row) {
                    ...
                    $equivalencias = explode(',', trim($row[1]));
                    foreach ($equivalencias as $eq){
                        $values = array();
                        array_push($values, $codigo_producto);
                        array_push($values, trim($eq));
                        array_push($equiv, array_combine($table_keys, array_values($values)));
                    }

                }

                if (count($equiv) > 1000) {
                    $m_rowsOk = array_chunk($equiv, 1000, true);
                    foreach ($m_rowsOk as $m_row) {
                        \DB::table('equivalencias')->insert($m_row);
                    }
                } else {
                    \DB::table('equivalencias')->insert($equiv);
                }
                $end = \Carbon\Carbon::now();
                $demoro = $start->diffInSeconds($end);
